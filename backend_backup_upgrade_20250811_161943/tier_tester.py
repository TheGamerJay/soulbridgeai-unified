...
This creates manual upgrade endpoints so you can test Growth and Max tiers.

Usage: python tier_tester.py
Then visit: http://127.0.0.1:5001/tier-test
"""
...
# Import the tier constants and functions from main app
try:
    from app import (
        FREE_LIMITS, PREMIUM_LIMITS, MAX_LIMITS, TIER_LIMITS,
        get_effective_feature_limit, get_effective_plan_for_display,
        get_effective_decoder_limits, get_effective_fortune_limits, 
        get_effective_horoscope_limits, check_trial_active_from_db
    )
    print("✅ Successfully imported tier functions from main app")
except ImportError as e:
    print(f"❌ Failed to import from main app: {e}")
...
@app.route("/")
def index():
    """Redirect to tier test page"""
    return redirect(url_for('tier_test_page'))
