...
# -----------------------------
def compatibility(a: str, b: str, rel_type: str = "romantic") -> Dict[str, Any]:
    ...
    if rel_type == "friendship":
        # Friendships are more forgiving - reduce penalties, boost certain combinations
        if eB in ELEMENT_GROUPS[eA]["challenging"]: score += 10  # Less penalty for friendship
        if A["element"] == "Air" or B["element"] == "Air": score += 5  # Air signs make good friends
        if A["modality"] == "Mutable" or B["modality"] == "Mutable": score += 3  # Adaptable friends
...
def generate_compatibility_interpretation(compat_data: Dict[str, Any]) -> str:
    ...
    # Score interpretation based on relationship type
    if rel_type == "friendship":
        if score >= 75:
...
def api_compat():
    ...
    try:
        return jsonify(compatibility(a, b, rel_type)), 200
    except Exception as e:
        return jsonify({"error": str(e)}), 400
...
@horo.post("/interpret-compat")
def api_interpret_compat():
    data = request.get_json(silent=True) or {}
    ...
    try:
        ...
        compat = compatibility(a, b, rel_type)
...
document.getElementById("compatGo").addEventListener("click", async () => {
  ...
  try {
    ...
    wrap.innerHTML = `<div class="compat-result">
      ...
      <div style="margin-top: 15px; text-align: center;">
        <button onclick="interpretCompatibility('${d.a}', '${d.b}', ${d.score}, '${d.type}')" style="font-size: 0.95rem; padding: 8px 16px;">💫 Why This Score? 💫</button>
      </div>
      <div id="compat-interpretation-${d.a}-${d.b}" style="display: none; margin-top: 15px; padding: 15px; background: rgba(0,0,0,0.3); border-radius: 10px; border-left: 4px solid #d4af37;">
        <div class="loading">✨ Analyzing cosmic compatibility factors...</div>
      </div>
    </div>`;
  } catch (error) {
