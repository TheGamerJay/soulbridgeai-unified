...
class AIContentFilter:
    ...
    def check_content(
        self, message: str, companion_name: str = "Blayzo"
            ...
            for keyword in keywords:
                if keyword in message_lower:
                    ...
                    if category == "crisis_intervention":
                        ...
                        logging.critical(
                            f"CRISIS INTERVENTION TRIGGERED: {keyword} detected from user"
                        )
                        return False, crisis_message

                    refusal = self._get_refusal_message(companion_name, category)
                    logging.warning(
                        f"Content filter triggered: {category} - keyword: {keyword}"
                    )
