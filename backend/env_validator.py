...
class EnvironmentValidator:
    ...
    def validate_optional_env_var(self, name: str, description: str = "") -> bool:
        ...
        if not value:
            self.warnings.append(
                f"Optional environment variable not set: {name} - {description}"
            )
            return False
        self.info.append(f"✓ {name} is set")
        return True
    ...
    def validate_api_key(self, key: str, min_length: int = 20) -> bool:
        ...
        return (
            len(key) >= min_length and key.replace("-", "").replace("_", "").isalnum()
        )
    ...
    def run_validation(self) -> Tuple[bool, Dict]:
        ...
        if db_url:
            if not self.validate_database_url(db_url):
                self.errors.append(
                    "DATABASE_URL format is invalid (should be postgresql://user:pass@host/db)"
                )
            else:
                self.info.append("✓ DATABASE_URL format is valid")
        else:
            self.validate_optional_env_var(
        ...
        if openai_key:
            ...
            self.validate_required_env_var(
        ...
        if stripe_secret:
            if not stripe_secret.startswith("sk_"):
                self.errors.append(
                    "STRIPE_SECRET_KEY format is invalid (should start with 'sk_')"
                )
            else:
        ...
        if stripe_public:
            if not stripe_public.startswith("pk_"):
                self.errors.append(
                    "STRIPE_PUBLIC_KEY format is invalid (should start with 'pk_')"
                )
            else:
                self.info.append("✓ STRIPE_PUBLIC_KEY format is valid")
        else:
            self.validate_optional_env_var(
                "STRIPE_PUBLIC_KEY", "Required for payment processing"
            )
        ...
        if sendgrid_key:
            if not sendgrid_key.startswith("SG."):
                self.warnings.append(
                    "SENDGRID_API_KEY format unusual (should start with 'SG.')"
                )
            else:
        ...
        if redis_url:
            ...
            self.validate_optional_env_var(
                "REDIS_URL", "Required for production rate limiting"
            )
...
def init_environment_validation(app):
    ...
    if not is_valid:
        logger.warning(
            "Starting with environment validation errors - check /debug/env-check"
        )
