...
class AnalyticsManager:
    ...
    def get_user_metrics(self, days: int = 30) -> Dict:
        ...
        try:
            ...
            return {
                ...
                'engagement_rate': round((daily_active / total_users * 100), 2) if total_users > 0 else 0,
        ...
        try:
            ...
            for companion in companions:
                companion_stats[companion] = {
                    'total_selections': 0,
                    'active_users': 0,
                    'is_premium': companion in ['Crimson', 'Violet', 'Blayzion', 'Blayzia'],
                    'estimated_conversations': 0
                }
            
            # Analyze user companion preferences
            for user in all_users:
                selected_companion = user.get('selectedCharacter') or user.get('favorite_companion')
            ...
            total_selections = sum(stats['total_selections'] for stats in companion_stats.values())
            
            if total_selections > 0:
                for companion in companions:
                    selection_ratio = companion_stats[companion]['total_selections'] / total_selections
                    companion_stats[companion]['estimated_conversations'] = int(total_conversations * selection_ratio)
            
            return {
                'period_days': days,
                'companions': companion_stats,
                'most_popular': max(companion_stats.keys(), key=lambda x: companion_stats[x]['total_selections']),
    ...
    def get_revenue_analytics(self, days: int = 30) -> Dict:
        ...
        try:
            ...
            # Calculate metrics
            total_revenue = estimated_monthly_revenue + premium_companion_purchases + switching_purchases
            ...
            return {
                ...
                'conversion_rate': round((premium_users / len(all_users) * 100), 2) if all_users else 0,
    ...
    def _get_conversation_metrics(self, days: int) -> Dict:
        ...
        return {
            ...
            'avg_conversations_per_user': round(estimated_total / max(1, len(self.db.users.get_all_users())), 1),
