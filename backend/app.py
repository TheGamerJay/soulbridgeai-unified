...
@app.route('/debug/reset-trial', methods=['POST'])
def reset_trial():
    """Reset trial for current user - FOR TESTING ONLY"""
    ...
    try:
        with get_db_connection() as conn:
            ...
            cursor.execute("""
                UPDATE users 
                SET trial_start_time = NULL, trial_used_permanently = FALSE 
                WHERE id = %s
            """, (user_id,))
            ...
            return jsonify({
                'success': True,
                'message': 'Trial reset successfully! You can now start a new 5-hour trial.',
                'user_id': user_id
            })
...
@app.route('/debug/trial-status')
def check_trial_status():
    """Check current trial status - FOR TESTING ONLY"""
    if 'user_id' not in session:
        return jsonify({'success': False, 'error': 'Not logged in'})
    ...
    try:
        with get_db_connection() as conn:
            cursor = conn.cursor()
            cursor.execute("""
                SELECT trial_start_time, trial_used_permanently 
                FROM users 
                WHERE id = %s
            """, (user_id,))
            ...
            status = {
                'success': True,
                'user_id': user_id,
                'trial_start_time': trial_start.isoformat() if trial_start else None,
                'trial_used_permanently': trial_used,
                'trial_currently_active': trial_active,
                'can_start_trial': not trial_used and not trial_active
            }
            
            if trial_start:
                time_elapsed = datetime.now(timezone.utc) - trial_start
                status['time_elapsed_hours'] = time_elapsed.total_seconds() / 3600
                status['time_remaining_hours'] = max(0, 5 - (time_elapsed.total_seconds() / 3600))
