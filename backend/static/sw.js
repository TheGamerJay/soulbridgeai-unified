...
const CACHE_NAME = `soulbridge-ai-v${Date.now()}`;
...
// Fetch event - network first, fallback to cache (prevents serving stale content during server issues)
...
function clearOldCaches() {
    return caches.keys().then(cacheNames => {
        return Promise.all(
            cacheNames.map(cacheName => {
                if (cacheName !== CACHE_NAME && cacheName.startsWith('soulbridge-ai-v')) {
...
function refreshCriticalResources() {
    const criticalResources = ['/static/css/animations.css', '/static/css/themes.css', '/static/js/mini-assistant-modal.js'];
    return Promise.all(
