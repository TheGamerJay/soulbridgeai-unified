#!/usr/bin/env python3
"""
Fix Railway PostgreSQL Database Schema Issues
Comprehensive fix for all database issues causing deployment failures
"""
...
def main():
    ...
    if not DATABASE_URL:
        ...
        print("This script requires Railway DATABASE_URL to fix PostgreSQL issues")
        return False
    ...
    try:
        ...
        try:
            ...
            if not columns:
                ...
                has_serial_id = any(col for col in columns if col[0] == 'id' and 'nextval' in str(col[3]))
                if not has_serial_id:
                    # Drop and recreate table with proper ID
                    cursor.execute("DROP TABLE tier_limits CASCADE")
                    cursor.execute("""
                        CREATE TABLE tier_limits (
                            id SERIAL PRIMARY KEY,
                            tier VARCHAR(20) NOT NULL,
        ...
        # 4. Ensure user_activity_log table exists (for admin utils)
        print("\n4. Ensuring user_activity_log table exists...")
        try:
            cursor.execute("""
            ...
            print("✅ Ensured user_activity_log table exists")
            conn.commit()
        except Exception as e:
        ...
        print("\n" + "=" * 50)
        print("✅ RAILWAY DATABASE SCHEMA FIX COMPLETED!")
        print("All identified issues have been resolved:")
        print("  - Added missing 'referrals' column to users table")
        print("  - Fixed tier_limits table with proper ID column")
        print("  - Standardized feature_usage table structure")
        print("  - Ensured user_activity_log table exists")
        print("  - Populated default tier limits")
